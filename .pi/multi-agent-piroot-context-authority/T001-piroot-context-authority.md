...
Implemented Controller-only Goal/Plan memory primitives/tools, canonical `.pi` context readers, managed-path guards, `.pi/.gitignore` tightening, and a tested one-time migration helper. Verification completed: focused/integrated MA3 and MA1/MA2 regressions **13 files / 56 tests passed**; `npm run check` passed, including Biome, TypeScript, dependency checks, and browser smoke.

Remaining
None. No runtime migration mode or legacy reader remains. MA3 checkpoint is committed locally as pending and will be pushed to `origin/main` per the user-authorized checkpoint; checkpoint e0e9098ee contains MA1/MA2.
