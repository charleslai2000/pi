...
function fixture(): { root: string; sessionDir: string; sessionId: string } {
	...
	writeFileSync(
		join(root, ".pi", "control", "goal-a", "T001-work.md"),
		"Status: ACTIVE\nWork area: design\nObjective: integrate\n",
	);
	writeFileSync(join(root, ".pi", "control", "goal-a", "T002-work.md"), "Status: READY\n");
	mkdirSync(join(root, ".pi", "control", "goal-b"), { recursive: true });
	writeFileSync(join(root, ".pi", "control", "goal-b", "goal.md"), "# Goal B\n");
	writeFileSync(join(root, ".pi", "control", "goal-b", "T004-work.md"), "Status: DONE\n");
	mkdirSync(join(root, ".pi", "control", "goal-c"), { recursive: true });
	writeFileSync(join(root, ".pi", "control", "goal-c", "goal.md"), "# Goal C\n");
	writeFileSync(join(root, ".pi", "control", "goal-c", "T002-work.md"), "Status: READY\n");
	setPiRoot(root);
	const sessionId = "d1";
...
describe("combined control view", () => {
	it("combines Task, frontier, assignment, and inactive runtime state without mutation", () => {
		...
		const before = readFileSync(join(value.root, ".pi", "control", "goal-a", "T001-work.md"));
		...
		expect(readFileSync(join(value.root, ".pi", "control", "goal-a", "T001-work.md"))).toEqual(before);
