...
- Added PiRoot-scoped Control Plane storage under `.pi/`, durable Controller/Executor roles and assignments, Session-native Task lifecycle with stop-envelope validation, Controller ControlEvents, managed-path mutation protection, and `/sessions` role/task projection. PiRoot discovery uses only `.pi/`; Task authority is optional and documented in ADR-0001.
