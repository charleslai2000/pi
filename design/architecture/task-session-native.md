...
Resolution is deterministic and side-effect-free: explicit `--root` wins; otherwise walk upward from the startup cwd and select the nearest ancestor containing `.pi/`. Initialization is separate: explicit `--root` requires an existing writable directory and initializes `.pi/`, `.pi/state/`, and `.pi/sessions/` idempotently. Interactive TTY startup with no discovered marker asks whether to initialize the current cwd; decline means standalone Pi. RPC/print/non-TTY startup never prompts or initializes implicitly and runs standalone when no marker exists. SDK/embedded callers may leave PiRoot unset. Never infer PiRoot from Git roots, `control/`, arbitrary project files, or session cwd after startup.
